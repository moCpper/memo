...
例如： 
		I PPPP ... PPBP I ....
**`以I帧开头到下一个I帧之前即两个I帧之间的距离，称之为GOP`**
...
根据日志可得，每六帧有一个关键帧，显而易见的大小也随之变大(**关键帧越多，压缩率越低**)

## PTS和DTS
****
 **PTS（Presentation Time Stamp）**

- **定义**：PTS是用于指示视频帧或音频样本应该何时被展示的时间戳。
- **用途**：确保媒体数据按照正确的时间顺序播放。例如，在视频播放中，即使某些帧由于网络延迟或其他原因到达得较晚，PTS也能确保它们在正确的时间被展示。
- **计算**：PTS通常基于帧率或采样率来计算。对于固定帧率的视频，PTS可以通过帧数乘以每帧的时间间隔来计算。

**DTS（Decode Time Stamp**

- **定义**：DTS是用于指示视频帧或音频样本应该何时被解码的时间戳。
- **用途**：确保数据包在正确的时间被解码，以便可以及时展示。DTS对于处理编码器产生的B帧特别重要，因为B帧可能需要参考未来的帧进行解码。
- **计算**：DTS的计算可能比PTS更复杂，因为它需要考虑解码顺序和解码依赖性。
 
 **场景**
- **视频播放**：在视频播放中，PTS用于确定何时显示每一帧，而DTS用于确定何时开始解码每一帧。
- **音频播放**：在音频播放中，PTS和DTS通常相同，因为音频通常是连续解码和播放的。